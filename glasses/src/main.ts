...
async function startGlassesMode(bridge: NonNullable<Awaited<ReturnType<typeof initDisplay>>>) {
  ...
  function publishScreen(state: AppState): void {
    try {
      const { header, body, footer, notice, card } = screenText(state)
      ...
      const session =
        cursor && !state.listOnNotifications
          ...
          : undefined
      const key = `${state.mode}\u0000${session?.id ?? ''}\u0000${session?.paneId ?? ''}\u0000${header}\u0000${notice ?? ''}\u0000${body}\u0000${footer}\u0000${card ? '1' : ''}`
      if (key === lastPublished) return
      lastPublished = key
      controller.ws.publishScreen({ header, body, footer, notice, card, mode: state.mode, session, at: Date.now() })
    } catch (err) {
