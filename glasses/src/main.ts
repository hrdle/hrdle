...
async function startGlassesMode(bridge: NonNullable<Awaited<ReturnType<typeof initDisplay>>>) {
   ...
   *
   * These four fields separate them. Off the head, in the case, or a dropped
   * BLE link are all ordinary reasons for the host to stop an app, and none of
   * them are bugs. What remains after they are excluded is the actual problem.
   *
  ...
  setupEvents(bridge, {
    ...
    onExit(kind) {
      ...
      //
      // And the device state, because a gesture cannot tell the two apart on
  ...
  void refreshDeviceNote() // so the first death has something to say too
  setInterval(() => {
