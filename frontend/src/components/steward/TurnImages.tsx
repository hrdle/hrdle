 ...
 */
export function TurnImages({ paths }: { paths: string[] }) {
	if (paths.length === 0) return null;
	return (
		<div className="mt-2 flex flex-wrap gap-2">
			{paths.map((path) => (
				<TurnImage key={path} path={path} />
			))}
		</div>
	);
}

function TurnImage({ path }: { path: string }) {
	...
	return (
		<button
			...
			className="block"
		>
			<img src={src} alt={name} className="max-h-48 rounded-md object-contain" />
		</button>
	);
}
