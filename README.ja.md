...
**Hrdle は herdr + handle。** herdr が、あなた自身のマシンの上でコーディングエージェント（Claude Code / Codex / Grok / Kimi）をターミナルのワークスペースとして動かします。Hrdle はそれを掴むための取っ手です。タブレットから、スマートフォンから、あるいは EVEN Realities G2 から。眺めるためのものではありません。キーボードの前でやることを、どこにいてもやるためのものです。

**マシンはヘッドレスで構いません。** ディスプレイもキーボードも、そこに座る人も不要です。Tailscale 越しに届くので、そのマシンに求めるのは起き続けていることだけです。
...
- **スマートグラス** - EVEN Realities G2 でセッションを読み、質問に答える（下記）
